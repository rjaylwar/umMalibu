...
public class VenmoLibrary {
    ...
    public static final int REQUEST_CODE_VENMO_APP_SWITCH = 99;
...
//                JSONArray response = (JSONArray) JSONValue.parse(data);
                JsonArray response = new JsonParser().parse(data).getAsJsonArray();
                JsonObject obj = response.get(0).getAsJsonObject();

                String payment_id = obj.get("payment_id").getAsString();
                String note = obj.get("note").getAsString();
                String amount = obj.get("amount").getAsString();
                String success = obj.get("success").getAsString();
